...
def test_marker_text_returns_rich_text_with_color() -> None:
    with patch(
        "openminion.cli.tui.presentation.markers.is_color_enabled",
        return_value=True,
    ):
...
def test_no_parallel_marker_definitions_in_streaming_or_transcript() -> None:
    import openminion.cli.tui.terminal.streaming as streaming_mod
    import openminion.cli.tui.terminal.transcript as transcript_mod
    from pathlib import Path

    streaming = Path(streaming_mod.__file__)
    transcript = Path(transcript_mod.__file__)

    streaming_src = streaming.read_text(encoding="utf-8")
    transcript_src = transcript.read_text(encoding="utf-8")
    assert "MARKER_TOOL_RUNNING =" not in streaming_src
    assert "MARKER_USER =" not in transcript_src
