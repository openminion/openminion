...
def test_terminal_discover_custom_commands_handles_bad_data_root(
    tmp_path: Path,
) -> None:
    from openminion.cli.tui.terminal.shell import _discover_custom_commands_for

    class _BadRoot:
        def __str__(self) -> str:
            raise ValueError("bad root")

    runtime = SimpleNamespace(api_runtime=SimpleNamespace(data_root=_BadRoot()))

    discovered = _discover_custom_commands_for(
        runtime=runtime,
...
def test_terminal_discover_custom_commands_handles_discovery_error(
    monkeypatch, tmp_path: Path
) -> None:
    import openminion.cli.tui.presentation.custom_commands as custom_commands_mod
    from openminion.cli.tui.terminal.shell import _discover_custom_commands_for

    def _boom(*_args, **_kwargs):
        raise RuntimeError("discovery failed")

    monkeypatch.setattr(custom_commands_mod, "discover_custom_commands", _boom)

    runtime = SimpleNamespace(api_runtime=SimpleNamespace(data_root=tmp_path))
    discovered = _discover_custom_commands_for(
        runtime=runtime,
        working_dir=str(tmp_path),
    )
