...
def test_repeated_file_mutation_closes_without_interpreting_user_prose() -> None:
    state = AdaptiveToolLoopState(
        messages=[
            Message(
                role="user",
...
def test_mutating_file_fallback_reports_structured_tool_facts() -> None:
    state = AdaptiveToolLoopState(
        messages=[
            Message(
                role="user",
...
def test_tool_evidence_closeout_does_not_infer_requested_labels() -> None:
    state = AdaptiveToolLoopState(
        messages=[
            Message(
                role="user",
    ...
    assert "tradeoffs:" not in text
    assert "recommendation:" not in text
    assert "result: tool budget exhausted." in text
