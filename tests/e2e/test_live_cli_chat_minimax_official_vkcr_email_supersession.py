...
def _stage_and_promote_user_email(
    *, memory_db: Path, session_id: str, email: str, agent_id: str
) -> None:
    service = MemoryService(SQLiteMemoryStore(memory_db))
    candidate_id = service.stage_candidate(
        scope=f"agent:{agent_id}",
        record_type="fact",
        title="User email address",
        content=email,
        tags=["live-proof", "vkcr"],
        confidence=1.0,
        meta={
            "normalized_key": "fact:user_email",
            "source": "vkcr_live_proof",
            "source_session_id": session_id,
        },
    )
    service.candidate_update(
        candidate_id,
        {
            "status": "approved",
            "confidence": 1.0,
    ...
    service.promote_candidate(candidate_id, f"agent:{agent_id}")


@pytest.mark.e2e
@pytest.mark.parametrize("agent_id", _AGENT_IDS)
def test_live_minimax_official_vkcr_email_supersession_cross_session(
    agent_id: str,
) -> None:
    require_live_flag()
    ...
    second = run_cli_session(
        session_id_prefix=f"{run_id}-s2",
        user_input=(f"My email is now {new_email}, not {old_email}.\n/exit\n"),
        agent_id=agent_id,
        config_path=_OFFICIAL_CONFIG,
        data_root_override=data_root,
    )
    ...
    recall = run_cli_session(
        ...
        user_input="What is my email? Answer with only the email address.\n/exit\n",
        agent_id=agent_id,
        config_path=_OFFICIAL_CONFIG,
        data_root_override=data_root,
    )
    ...
    assert old_email.lower() not in assistant_text, (
        "live MiniMax recall leaked the superseded email\n"
        f"transcript={recall.transcript_path}\n"
        f"assistant_text={assistant_text}"
    )
