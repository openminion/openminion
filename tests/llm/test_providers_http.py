...
class ProviderHTTPTests(unittest.TestCase):
    ...
    def test_openai_and_anthropic_materialize_artifact_images_through_common_owner(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ref = artifactctl.ingest_bytes(png_bytes, mime="image/png").ref
            finally:
                artifactctl.close()
            ...
            def _fake_urlopen(http_request, timeout=None):
                ...
                payload = (
                    anthropic_payload
                    if "anthropic.com" in http_request.full_url
                    else openai_payload
                )
                return _FakeHTTPResponse(payload)
            ...
            with (
                ...
                openai_response = OpenAIProvider().complete(
                    request,
                    {
                        "api_key": "test-key",
                        "base_url": "https://api.openai.com/v1",
                ...
                anthropic_response = AnthropicProvider().complete(
                    request,
                    {
                        "api_key": "test-key",
                        "base_url": "https://api.anthropic.com/v1",
        ...
        self.assertTrue(anthropic_response.ok)
        openai_url = bodies[0]["messages"][0]["content"][1]["image_url"]["url"]
        self.assertEqual(base64.b64decode(openai_url.split(",", 1)[1]), png_bytes)
        anthropic_source = bodies[1]["messages"][0]["content"][1]["source"]
        self.assertEqual(anthropic_source["media_type"], "image/png")
        self.assertEqual(base64.b64decode(anthropic_source["data"]), png_bytes)

    def test_artifact_image_schema_rejects_cross_source_fields(self) -> None:
        ref = "artifact://sha256/" + "a" * 64
        with self.assertRaises(ValueError):
            ImageContentPart(source="path", path="shot.png", artifact_ref=ref)
        with self.assertRaises(ValueError):
            ImageContentPart(
                source="artifact",
                artifact_ref=ref,
    ...
    def test_common_artifact_aggregate_rejects_more_than_four_images(self) -> None:
        parts = [
            ImageContentPart(
                source="artifact",
                artifact_ref="artifact://sha256/" + value * 64,
                mime_type="image/png",
            )
            for value in "abcde"
        ]
        request = LLMRequest(
            model="vision-model",
            messages=[Message(role="user", content_parts=parts)],
        )
        with patch(
            "openminion.modules.llm.providers.message_payloads.inspect_artifact_image",
            return_value=("image/png", 1),
        ):
            for provider, base_url in (
                (OpenAIProvider(), "https://api.openai.com/v1"),
                (AnthropicProvider(), "https://api.anthropic.com/v1"),
            ):
                with self.assertRaisesRegex(LLMCtlError, "count exceeds"):
                    provider.complete(
                        request,
                        {
                            "api_key": "test-key",
        ...
        byte_limited = request.model_copy(
            update={"messages": [Message(role="user", content_parts=parts[:3])]}
        )
        with patch(
            "openminion.modules.llm.providers.message_payloads.inspect_artifact_image",
            return_value=("image/png", 8 * 1024 * 1024),
        ):
            with self.assertRaisesRegex(LLMCtlError, "bytes exceed"):
