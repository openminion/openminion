...
def test_artifact_catalog_pages_join_terminal_to_exact_requested_parent(
    store: SQLiteSessionStore,
) -> None:
    session_id = _session(store)
    requested_id = store.append_event(
    ...
    terminal_id = store.append_event(
        session_id,
        event_type="tool.call.completed",
        parent_event_id=requested_id,
        payload={
    ...
    while not pages[-1]["complete"]:
        pages.append(
            store.get_artifact_catalog_event_page(
                session_id,
                after_seq=pages[-1]["next_after_seq"],
    ...
    terminal = next(
        ...
        if event["event_id"] == terminal_id
    )

    assert first["complete"] is False
    assert pages[-1]["complete"] is True
    assert terminal["tool_parent"] == {
