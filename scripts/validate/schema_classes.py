#!/usr/bin/env python3
"""Validate schema-heavy files against the schema class-count baseline."""
...
def load_baseline(path: Path) -> dict[str, BaselineEntry]:
    ...
    try:
        ...
        raise SystemExit(
            f"validate_schema_class_count: cannot read baseline {path}: {exc}"
        ) from exc
    for line_number, raw_line in enumerate(lines, start=1):
        ...
        if len(parts) != 3:
            raise SystemExit(
                f"validate_schema_class_count: malformed baseline line {line_number}: "
                "expected path<TAB>class_count<TAB>reason"
            )
        rel_path, raw_count, reason = (part.strip() for part in parts)
        ...
        try:
            ...
            raise SystemExit(
                f"validate_schema_class_count: malformed baseline line {line_number}: "
                f"invalid class count {raw_count!r}"
            ) from exc
        entries[rel_path] = BaselineEntry(
            path=rel_path,
...
def validate(
    *,
    ...
    at_or_over_ceiling_count = 0
    checked_count = 0

    for path in source_files(source_root):
        checked_count += 1
        rel_path = path.relative_to(repo_root).as_posix()
        class_count = count_top_level_classes(path)
        entry = baseline.get(rel_path)
        if class_count >= ceiling:
            at_or_over_ceiling_count += 1
            if entry is None:
                findings.append(
                    Finding(
                        code="new_over_ceiling_schema",
                        path=rel_path,
                        detail=(
                            f"{class_count} top-level classes meets or exceeds "
                            f"ceiling {ceiling}"
                        ),
                    )
                )
            ...
            if class_count > entry.class_count:
                findings.append(
                    Finding(
                        code="baselined_schema_grew",
                        path=rel_path,
                        detail=(
                            f"{class_count} top-level classes exceeds baseline "
                            f"{entry.class_count}"
                        ),
                    )
                )
        ...
        if entry is not None:
            ...
            findings.append(
                Finding(
                    ...
                    path=rel_path,
                    detail=f"{class_count} top-level classes is below ceiling {ceiling}",
                )
            )
    ...
    metrics = {
        ...
        "ceiling": ceiling,
        "at_or_over_ceiling": at_or_over_ceiling_count,
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        "validator": "validate_schema_class_count",
        "ok": not findings,
        "metrics": metrics,
        "findings": [
    ...
    emit_json_report(
        "validate_schema_class_count",
        payload,
        summary=(
            ("checked", metrics["checked"]),
            ("ceiling", metrics["ceiling"]),
            ("at/over ceiling", metrics["at_or_over_ceiling"]),
