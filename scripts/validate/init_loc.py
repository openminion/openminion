#!/usr/bin/env python3
"""Validate OpenMinion __init__.py files against the init-file LOC baseline."""
...
DEFAULT_BASELINE = REPO_ROOT / "scripts" / "baselines" / "init_file_loc_baseline.tsv"
...
def count_loc(path: Path) -> int:
    try:
        ...
        raise SystemExit(f"validate_init_file_loc: cannot read {path}: {exc}") from exc
...
def load_baseline(path: Path) -> dict[str, BaselineEntry]:
    ...
    try:
        ...
        raise SystemExit(
            f"validate_init_file_loc: cannot read baseline {path}: {exc}"
        ) from exc
    for line_number, raw_line in enumerate(lines, start=1):
        ...
        if len(parts) != 3:
            raise SystemExit(
                f"validate_init_file_loc: malformed baseline line {line_number}: "
...
def init_files(source_root: Path) -> list[Path]:
    return sorted(path for path in source_root.rglob("__init__.py") if path.is_file())
...
def validate(
    *,
    ...
    for path in init_files(source_root):
        checked_count += 1
        ...
        if loc >= ceiling:
            ...
            if entry is None:
                findings.append(
                    Finding(
                        code="new_over_ceiling_init",
                        path=rel_path,
                        detail=f"{loc} LOC meets or exceeds ceiling {ceiling}",
                    )
                )
                continue
            seen.add(rel_path)
        ...
        if entry is not None:
            ...
            findings.append(
                Finding(
                    ...
                    path=rel_path,
                    detail=f"{loc} LOC is below ceiling {ceiling}",
                )
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        "validator": "validate_init_file_loc",
        "ok": not findings,
        "metrics": metrics,
        "findings": [
