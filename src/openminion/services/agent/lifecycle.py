...
# Hook callable signature: receives a LifecycleEvent + HookContext;
# return value is ignored under Gate E = E1 (observe-only).
LifecycleHook = Callable[["LifecycleEvent", HookContext], None]


class LifecycleHookRegistry:
    """Programmatic-only lifecycle hook registry (EVP-01 v1)."""
    ...
    def fire(
        ...
        Each hook's exception is caught + logged so misbehaving
        hooks cannot break tool dispatch or session lifecycle.
        Per Gate E = E1, return values are ignored.
        """
...
def _resolve_hook_context_for_lifecycle(
    *,
    ...
    """Build a `HookContext` for lifecycle firing.

    Default-safe: when callers don't have a `HookContext` in scope
    (e.g. tool dispatch deep inside the brain runner), we accept
    optional overrides.
    """
    if logger is None:
        logger = logging.getLogger("openminion.lifecycle_hook")
    return HookContext(config=config, logger=logger)  # type: ignore[arg-type]
...
def fire_lifecycle_event(
    ...
    *,
    ...
    checking registry state. Per Gate E = E1, hooks observe only.
    """
