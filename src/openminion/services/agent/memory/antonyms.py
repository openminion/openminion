...
def load_antonym_pairs(config_path: str | Path) -> set[frozenset[str]]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("memory antonyms payload must be a mapping")
    pairs_payload = payload.get("pairs") or []
    if not isinstance(pairs_payload, list):
        raise ValueError("memory antonyms pairs must be a list")
    ...
    if not pairs:
        raise ValueError("memory antonyms config did not contain any valid pairs")
    return pairs
