...
class HookManifestError(RuntimeError):
...
def validate_plugin_manifest(payload: Mapping[str, Any]) -> HookManifest:
    errors: list[str] = []
    ...
    if errors:
        raise HookManifestError(errors)
...
def load_plugin_manifest(path: str | Path) -> HookManifest:
    manifest_path = Path(path)
    try:
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    except FileNotFoundError as exc:
        raise HookManifestError([f"manifest not found: {manifest_path}"]) from exc
    except json.JSONDecodeError as exc:
        raise HookManifestError([f"manifest is not valid JSON: {exc}"]) from exc
    if not isinstance(payload, dict):
        raise HookManifestError(["manifest root must be an object."])
