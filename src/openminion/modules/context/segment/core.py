...
def append_summary_segments(
    ...
    *,
    ...
    if compression_enabled and seed_text and seed_text.strip():
        seed_block = runtime.fit_section(
            "seed_block",
            seed_text,
            runtime.budgets.summary_tokens,
        )
        if seed_block.strip():
            runtime.segments.append(
                runtime.make("seed_block", "summaries", f"[CONTEXT SEED]\n{seed_block}")
            )
            summary_items += 1
    ...
    compression_ref = None
    if compression_enabled and compressctl is not None:
        try:
            ...
            if snapshot:
                compression_ref = (
                    session_slice.seed_bundle_id or session_slice.checkpoint_id
                )
        except Exception:
            pass
    if (
    ...
    summary_cap_tokens = runtime.budgets.summary_tokens
    session_work_summary_cap_tokens = runtime.budgets.summary_tokens
    if summary_text.strip() and session_work_summary_text:
        session_work_summary_cap_tokens = max(40, runtime.budgets.summary_tokens // 2)
        summary_cap_tokens = max(
            40, runtime.budgets.summary_tokens - session_work_summary_cap_tokens
        )
    summary_block = runtime.fit_section("summary", summary_text, summary_cap_tokens)
    archive_added = False
    if summary_block.strip():
        runtime.segments.append(
            runtime.make("summary", "summaries", f"[SESSION SUMMARY]\n{summary_block}")
        )
        summary_items += 1
        if compression_ref:
            runtime.segments.append(
                runtime.make(
                    "compression_ref",
                    "summaries",
                    f"[COMPRESSION REFERENCE]\ncheckpoint: {compression_ref}",
                )
            )
            summary_items += 1
    session_work_summary_block = runtime.fit_section(
        "session_work_summary",
        session_work_summary_text,
        session_work_summary_cap_tokens,
    )
    if session_work_summary_block.strip():
        runtime.segments.append(
            runtime.make(
                "session_work_summary",
                "summaries",
                f"[SESSION WORK SUMMARY]\n{session_work_summary_block}",
            )
        )
        summary_items += 1
    if session_slice.archive_refs:
        archive_text = (
            "Compaction archive refs (full transcript chunks):\n"
            + "\n".join(f"- {ref}" for ref in session_slice.archive_refs[:5])
        )
        archive_block = runtime.fit_section(
            "archive_refs",
            archive_text,
            runtime.budgets.summary_tokens // 4,
        )
        if archive_block.strip():
            runtime.segments.append(
                runtime.make("archive_refs", "summaries", archive_block)
            )
            archive_added = True
    if archive_added:
        summary_items += 1
