...
class RunStats:
    ...
    def from_mapping(cls, payload: Mapping[str, Any] | None) -> "RunStats | None":
        ...
        stats = cls(
            input_tokens=coerce_non_negative_int(input_tokens),
            output_tokens=coerce_non_negative_int(output_tokens),
            cache_read_tokens=coerce_non_negative_int(cache_read_tokens),
            llm_calls=coerce_non_negative_int(llm_calls),
            tool_calls=coerce_non_negative_int(tool_calls),
            tool_errors=coerce_non_negative_int(
                tool_errors
                if tool_errors is not None
                else _tool_error_count_from_payload(payload)
            ),
            duration_ms=coerce_non_negative_int(duration_ms),
        )
