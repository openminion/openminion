...
class SchemaDriftKind(str, Enum):
    """Closed vocabulary of structural drift kinds."""
...
def _normalize_type(raw: str) -> str:
    """Normalize a SQLite type string for case-insensitive comparison."""
...
def derive_expected_schema(
    ...
    """Build the expected schema by replaying migrations in memory."""
...
def _read_live_tables(
    ...
    """Read the live schema in the expected-schema shape."""
...
def _diff_tables(
    *,
    ...
    expected_by_name = {t.name: t for t in expected_tables}
    observed_by_name = {
...
def _diff_columns(
    ...
    expected_by_name = {c.name: c for c in expected_table.columns}
    observed_by_name = {c.name: c for c in observed_table.columns}
    findings: list[SchemaDriftFinding] = []
    for name in sorted(expected_by_name.keys() | observed_by_name.keys()):
