...
from .plan_control import completable_active_plan_id, unresolved_active_plan_step_ids
...
class AdaptiveLoopRunnerClosureMixin:
    ...
    def _maybe_force_duplicate_closure(
        self,
        *,
        tool_calls: list[Any],
        signature: str,
        ...
        outcome, duration_ms, tokens_used = _force_duplicate_batch_answer_only_closure(
            loop_ctx=self.loop_ctx,
            profile=self.profile,
            loop_state=self.loop_state,
            runtime=self.runtime,
        ...
        return self._finalize_answer_only_closure_outcome(
            ...
            tokens_used=(
                prepared.iter_input_tokens + prepared.iter_output_tokens + tokens_used
            ),
        )
    ...
    def _handle_duplicate_signature(
        ...
        *,
        ...
        plan_closeout_pending = bool(
            unresolved_active_plan_step_ids(self.loop_ctx)
            or completable_active_plan_id(self.loop_ctx)
        )
        if duplicate_retry_count < _DUPLICATE_BATCH_RECOVERY_LIMIT:
            ...
            if eligible_facts is not None and not plan_closeout_pending:
                duplicate_outcome = self._maybe_force_duplicate_closure(
                    tool_calls=tool_calls,
                    signature=signature,
                    prepared=prepared,
                )
