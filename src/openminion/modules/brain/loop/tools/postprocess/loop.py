...
def _update_prefetch_state(
    *,
    ...
    if prefetch_predictor is None:
        return prefetch_pending
    iter_tool_names = [str(getattr(tc, "name", "") or "").strip() for tc in tool_calls]
...
def _record_cache_and_iteration_profile(
    *,
    profile: AdaptiveToolLoopProfile,
    loop_state: Any,
    loop_cache: Any,
...
def _persist_loop_snapshot(
    *,
    ...
    if ctx_state is None or not hasattr(ctx_state, STATE_KEY_MODULE_STATE):
        return
    module_state = dict(getattr(ctx_state, STATE_KEY_MODULE_STATE, {}) or {})
    module_state["adaptive_loop"] = _loop_snapshot_from_state(
        profile=profile,
...
def finalize_iteration_state(
    ...
    *,
    ...
    prefetch_pending = _update_prefetch_state(
        profile=profile,
        loop_state=loop_state,
        tool_calls=tool_calls,
    ...
    _record_cache_and_iteration_profile(
        profile=profile,
        loop_state=loop_state,
