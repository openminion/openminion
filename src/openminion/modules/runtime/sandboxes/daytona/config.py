...
class DaytonaConfig:
    ...
    def resolve_api_key(self, env: Mapping[str, str] | None = None) -> str:
        ...
        return source.get(self.api_key_env_var, "").strip()
    ...
    def from_environment(
        ...
        source = env or {}
        endpoint = source.get("OPENMINION_DAYTONA_ENDPOINT", "").strip()
        if not endpoint:
            return None
        connect_timeout = float(
            source.get("OPENMINION_DAYTONA_CONNECT_TIMEOUT_S", "").strip() or 5.0
        )
        command_timeout = float(
            source.get("OPENMINION_DAYTONA_COMMAND_TIMEOUT_S", "").strip() or 30.0
        )
