...
PINCHTAB_VERSION_ENV = "OPENMINION_PINCHTAB_VERSION"
...
class PinchTabBinaryResolver:
    ...
    def _cached_resolution(self) -> PinchTabBinaryResolution:
        ...
        verified = _cached_binary_verified(binary, self.cfg.sha256, release_dir)
        return PinchTabBinaryResolution(
            binary_path=binary,
            version=self.cfg.version,
            platform_id=_platform_id(self.cfg.platform_id),
    ...
    def _download_and_resolve(self) -> PinchTabBinaryResolution:
        if not self.cfg.download_url:
            raise PinchTabBinaryError(
                "PINCHTAB_DOWNLOAD_URL_MISSING",
                "PinchTab download requires OPENMINION_PINCHTAB_DOWNLOAD_URL until release asset discovery is configured.",
            )
        ...
        binary = release_dir / _BINARY_NAME
        try:
            self._downloader(self.cfg.download_url, binary)
            _make_executable(binary)
            if self.cfg.sha256 and not _sha256_matches(binary, self.cfg.sha256):
                raise PinchTabBinaryError(
                    "PINCHTAB_CHECKSUM_MISMATCH",
                    "Downloaded PinchTab checksum did not match configured sha256.",
                )
            self._write_manifest(binary=binary, verified=bool(self.cfg.sha256))
        except PinchTabBinaryError as exc:
    ...
    def _write_manifest(self, *, binary: Path, verified: bool) -> None:
        payload = {
            "version": self.cfg.version,
...
def _cached_binary_verified(
    ...
    checksum_path = release_dir / _CHECKSUM_NAME
    if not checksum_path.exists():
        return False
    expected = checksum_path.read_text(encoding="utf-8").split()[0]
    return _sha256_matches(binary, expected)
